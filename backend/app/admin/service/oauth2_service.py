        ...
        async with async_db_session.begin() as db:
            ...
            if not sys_user:
                ...
                new_sys_user = RegisterUserParam(
                    username=_username, password=None, nickname=_nickname, email=_email
                )
            ...
            if not user_social:
                new_user_social = CreateUserSocialParam(
                    source=social.value, uid=str(_id), user_id=sys_user_id
                )
                await user_social_dao.create(db, new_user_social)
            access_token = await jwt.create_access_token(
                str(sys_user_id), sys_user.is_multi_login
            )
            refresh_token = await jwt.create_refresh_token(
                str(sys_user_id), multi_login=sys_user.is_multi_login
            )
